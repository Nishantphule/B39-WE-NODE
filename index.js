...
// get all movies
  app.get("/movies", async function (request, response) {
    const movies = await client.db("b39we").collection("movies").find({}).toArray()
...
// get movies by id
  app.get("/movies/:id", async function (request, response) {
    ...
    // from mongodb
    const movie = await client.db("b39we").collection("movies").findOne({id:id})
    movie 
    ? response.send(movie) 
...
app.delete("/movies/:id", async function (request, response) {
  ...
  // from mongodb
  const movie = await client.db("b39we").collection("movies").deleteOne({id:id});
